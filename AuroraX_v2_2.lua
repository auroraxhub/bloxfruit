...
-- =====================================================================
-- AuroraLib v2 : compact, symmetric purple UI (by Aurora X Hub)
-- =====================================================================
local redzlib do
    local TweenService     = game:GetService("TweenService")
    ...
    local T = {
        Bg        = Color3.fromRGB(14, 8, 26),
        Panel     = Color3.fromRGB(22, 14, 40),
        Panel2    = Color3.fromRGB(30, 20, 55),
        Stroke    = Color3.fromRGB(120, 70, 240),
        StrokeSoft= Color3.fromRGB(70, 45, 130),
        Accent    = Color3.fromRGB(170, 110, 255),
        Accent2   = Color3.fromRGB(110, 60, 220),
        Text      = Color3.fromRGB(235, 225, 255),
        SubText   = Color3.fromRGB(170, 155, 210),
        Muted     = Color3.fromRGB(120, 105, 160),
    }
    ...
    local function getParent()
        ...
        pcall(function() CoreGui:GetChildren() end)
        return CoreGui
    end
    ...
    local function stroke(c,t) return new("UIStroke",{Color=c or T.Stroke, Thickness=t or 1, ApplyStrokeMode=Enum.ApplyStrokeMode.Border}) end
    local function pad(all)
        return new("UIPadding",{PaddingTop=UDim.new(0,all),PaddingBottom=UDim.new(0,all),PaddingLeft=UDim.new(0,all),PaddingRight=UDim.new(0,all)})
    end
    local function tween(o,t,p) return TweenService:Create(o, TweenInfo.new(t or 0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), p) end

    local function isImageIcon(s)
        return type(s)=="string" and (s:sub(1,4)=="rbxa" or s:sub(1,4)=="rbxt" or s:sub(1,7)=="http://" or s:sub(1,8)=="https://")
    end
    local ICON_MAP = { Info="i", waves="~", tent="^", locate=">", farming="*" }
    ...
    -- Notifications ------------------------------------------------------
    local notifRoot
    local function ensureNotif(parent)
        if notifRoot and notifRoot.Parent then return notifRoot end
        local gui = new("ScreenGui",{Name="AuroraNotif",ResetOnSpawn=false,ZIndexBehavior=Enum.ZIndexBehavior.Sibling,IgnoreGuiInset=true,Parent=parent})
        pcall(function() gui.DisplayOrder = 9999 end)
        notifRoot = new("Frame",{Name="Holder",BackgroundTransparency=1,AnchorPoint=Vector2.new(1,1),Position=UDim2.new(1,-16,1,-16),Size=UDim2.new(0,280,1,-32),Parent=gui})
        new("UIListLayout",{FillDirection=Enum.FillDirection.Vertical,HorizontalAlignment=Enum.HorizontalAlignment.Right,VerticalAlignment=Enum.VerticalAlignment.Bottom,Padding=UDim.new(0,8),SortOrder=Enum.SortOrder.LayoutOrder,Parent=notifRoot})
        return notifRoot
    end

    local Lib = {}; Lib.__index = Lib

    function Lib:MakeNotification(opts)
        opts = opts or {}
        local parent = getParent()
        local holder = ensureNotif(parent)
        local card = new("Frame",{BackgroundColor3=T.Panel,Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,Parent=holder,ClipsDescendants=true})
        corner(10).Parent = card; stroke(T.Stroke,1).Parent = card; pad(10).Parent = card
        new("UIListLayout",{Padding=UDim.new(0,4),Parent=card})
        new("TextLabel",{BackgroundTransparency=1,Size=UDim2.new(1,0,0,18),Font=Enum.Font.GothamBold,TextSize=14,TextColor3=T.Accent,TextXAlignment=Enum.TextXAlignment.Left,Text=tostring(opts.Name or opts.Title or "Notification"),Parent=card})
        new("TextLabel",{BackgroundTransparency=1,Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,Font=Enum.Font.Gotham,TextSize=13,TextColor3=T.Text,TextWrapped=true,TextXAlignment=Enum.TextXAlignment.Left,Text=tostring(opts.Content or opts.Message or ""),Parent=card})
        task.delay(opts.Time or 3, function() pcall(function() card:Destroy() end) end)
    end
    ...
    function Lib:MakeWindow(cfg)
        ...
        for _,g in ipairs(parent:GetChildren()) do
            if g.Name == "AuroraXHub" or g.Name == "AuroraXMini" then pcall(function() g:Destroy() end) end
        end

        local gui = new("ScreenGui",{Name="AuroraXHub",ResetOnSpawn=false,ZIndexBehavior=Enum.ZIndexBehavior.Sibling,IgnoreGuiInset=true,Parent=parent})
        pcall(function() gui.DisplayOrder = 500 end)

        local scaleRoot = new("Frame",{BackgroundTransparency=1,Size=UDim2.fromScale(1,1),Parent=gui})
        local uiScale   = new("UIScale",{Scale=1,Parent=scaleRoot})

        -- Compact sizing
        local vp = workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize or Vector2.new(1280,720)
        local isSmall = vp.X < 800
        local winW, winH
        if isSmall then
            winW = math.clamp(math.floor(vp.X * 0.82), 300, 420)
            winH = math.clamp(math.floor(vp.Y * 0.62), 320, 460)
        else
            winW, winH = 500, 360
        end

        local win = new("Frame",{
            Name="Window", BackgroundColor3=T.Bg,
            Size=UDim2.new(0,winW,0,winH),
            Position=UDim2.new(0.5,-math.floor(winW/2),0.5,-math.floor(winH/2)),
            Parent=scaleRoot, ClipsDescendants=true,
        })
        corner(14).Parent = win
        stroke(T.Stroke,1.4).Parent = win
        new("UIGradient",{Rotation=125,Color=ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(24,14,48)),
            ColorSequenceKeypoint.new(1, Color3.fromRGB(10,6,20)),
        }),Parent=win})

        -- Top bar (title on top row, subtitle right below, buttons vertically centered)
        local TOP_H = 44
        local top = new("Frame",{Name="Top",BackgroundColor3=T.Panel,Size=UDim2.new(1,0,0,TOP_H),Parent=win,BorderSizePixel=0})
        corner(14).Parent = top
        new("Frame",{BackgroundColor3=T.Panel,BorderSizePixel=0,Position=UDim2.new(0,0,1,-14),Size=UDim2.new(1,0,0,14),Parent=top})
        new("Frame",{BackgroundColor3=T.Stroke,BorderSizePixel=0,BackgroundTransparency=0.6,Position=UDim2.new(0,10,1,-1),Size=UDim2.new(1,-20,0,1),Parent=top})

        new("TextLabel",{BackgroundTransparency=1,Position=UDim2.new(0,12,0,4),Size=UDim2.new(1,-84,0,18),Font=Enum.Font.GothamBold,TextSize=14,TextColor3=T.Accent,TextXAlignment=Enum.TextXAlignment.Left,TextYAlignment=Enum.TextYAlignment.Center,Text=tostring(cfg.Title or "Aurora X Hub"),Parent=top})
        new("TextLabel",{BackgroundTransparency=1,Position=UDim2.new(0,12,0,22),Size=UDim2.new(1,-84,0,16),Font=Enum.Font.Gotham,TextSize=11,TextColor3=T.SubText,TextXAlignment=Enum.TextXAlignment.Left,TextYAlignment=Enum.TextYAlignment.Center,Text=tostring(cfg.SubTitle or ""),Parent=top})

        local function mkTopBtn(txt, color, xOffset)
            local b = new("TextButton",{
                BackgroundColor3=color, AnchorPoint=Vector2.new(1,0.5),
                Position=UDim2.new(1,xOffset,0.5,0), Size=UDim2.new(0,24,0,24),
                Font=Enum.Font.GothamBold, TextSize=14, TextColor3=Color3.new(1,1,1),
                Text=txt, AutoButtonColor=true, Parent=top, BorderSizePixel=0,
            })
            corner(6).Parent = b
            return b
        end
        local closeBtn = mkTopBtn("x", Color3.fromRGB(200,60,90), -10)
        local miniBtn  = mkTopBtn("-", T.Accent2, -40)
        drag(win, top)

        -- Body
        local body = new("Frame",{BackgroundTransparency=1,Position=UDim2.new(0,0,0,TOP_H),Size=UDim2.new(1,0,1,-TOP_H),Parent=win})
        pad(8).Parent = body

        local sideW = isSmall and 128 or 150
        local side = new("ScrollingFrame",{Name="Side",BackgroundColor3=T.Panel,Size=UDim2.new(0,sideW,1,0),Position=UDim2.new(0,0,0,0),BorderSizePixel=0,ScrollBarThickness=0,ScrollingDirection=Enum.ScrollingDirection.Y,CanvasSize=UDim2.new(0,0,0,0),AutomaticCanvasSize=Enum.AutomaticSize.Y,Parent=body})
        corner(10).Parent = side
        stroke(T.StrokeSoft,1).Parent = side
        new("UIListLayout",{Padding=UDim.new(0,4),SortOrder=Enum.SortOrder.LayoutOrder,Parent=side})
        new("UIPadding",{PaddingTop=UDim.new(0,6),PaddingBottom=UDim.new(0,6),PaddingLeft=UDim.new(0,6),PaddingRight=UDim.new(0,6),Parent=side})

        local content = new("Frame",{Name="Content",BackgroundTransparency=1,Position=UDim2.new(0,sideW+8,0,0),Size=UDim2.new(1,-(sideW+8),1,0),Parent=body})

        local Window = {}
        Window._tabs = {}
        Window._activeTab = nil
        Window._gui = gui
        Window._scale = uiScale

        function Window:SetUIScale(v) uiScale.Scale = tonumber(v) or 1 end

        local function selectTab(tab)
            if Window._activeTab == tab then return end
            for _,t in ipairs(Window._tabs) do
                t._button.BackgroundColor3 = T.Panel2
                t._label.TextColor3 = T.SubText
                t._page.Visible = false
                if t._stroke then t._stroke.Transparency = 1 end
            end
            tab._button.BackgroundColor3 = T.Accent2
            tab._label.TextColor3 = Color3.new(1,1,1)
            if tab._stroke then tab._stroke.Transparency = 0 end
        ...
        function Window:MakeTab(tcfg)
            tcfg = tcfg or {}
            local Tab = {}
            local btn = new("TextButton",{BackgroundColor3=T.Panel2,Size=UDim2.new(1,0,0,30),AutoButtonColor=false,Text="",BorderSizePixel=0,Parent=side})
            corner(8).Parent = btn
            local st = stroke(T.Stroke,1); st.Transparency=1; st.Parent = btn
            Tab._stroke = st

            local ix = 8
            local hasImg = tcfg.Icon and isImageIcon(tcfg.Icon)
            if hasImg then
                new("ImageLabel",{BackgroundTransparency=1,Position=UDim2.new(0,ix,0.5,-8),Size=UDim2.new(0,16,0,16),Image=tcfg.Icon,ImageColor3=T.Accent,Parent=btn})
                ix = ix + 20
            else
                local iconTxt = ICON_MAP[tostring(tcfg.Icon or ""):lower()] or "-"
                new("TextLabel",{BackgroundTransparency=1,Position=UDim2.new(0,ix,0,0),Size=UDim2.new(0,14,1,0),Font=Enum.Font.GothamBold,TextSize=13,TextColor3=T.Accent,Text=iconTxt,Parent=btn})
                ix = ix + 18
            end
            local lbl = new("TextLabel",{BackgroundTransparency=1,Position=UDim2.new(0,ix,0,0),Size=UDim2.new(1,-ix-6,1,0),Font=Enum.Font.GothamSemibold,TextSize=11,TextColor3=T.Text,TextXAlignment=Enum.TextXAlignment.Left,TextYAlignment=Enum.TextYAlignment.Center,TextTruncate=Enum.TextTruncate.AtEnd,Text=tostring(tcfg.Title or "Tab"),Parent=btn,Name="Label"})
            Tab._label = lbl

            local page = new("ScrollingFrame",{BackgroundTransparency=1,Size=UDim2.new(1,0,1,0),BorderSizePixel=0,Visible=false,ScrollBarThickness=2,ScrollBarImageColor3=T.Accent2,ScrollingDirection=Enum.ScrollingDirection.Y,CanvasSize=UDim2.new(0,0,0,0),AutomaticCanvasSize=Enum.AutomaticSize.Y,Parent=content})
            new("UIListLayout",{Padding=UDim.new(0,6),SortOrder=Enum.SortOrder.LayoutOrder,Parent=page})
            new("UIPadding",{PaddingTop=UDim.new(0,2),PaddingBottom=UDim.new(0,10),PaddingLeft=UDim.new(0,2),PaddingRight=UDim.new(0,6),Parent=page})
            ...
            local function baseCard(h)
                local f = new("Frame",{BackgroundColor3=T.Panel,Size=UDim2.new(1,-4,0,h or 34),Parent=page,BorderSizePixel=0})
                corner(8).Parent = f
                stroke(T.StrokeSoft,1).Parent = f
                return f
            end

            function Tab:AddSection(name)
                local f = new("Frame",{BackgroundTransparency=1,Size=UDim2.new(1,-4,0,24),Parent=page})
                new("TextLabel",{BackgroundTransparency=1,Size=UDim2.new(1,-8,1,-2),Position=UDim2.new(0,4,0,0),Font=Enum.Font.GothamBold,TextSize=12,TextColor3=T.Accent,TextXAlignment=Enum.TextXAlignment.Left,TextYAlignment=Enum.TextYAlignment.Center,Text=tostring(name or ""),Parent=f})
                new("Frame",{BackgroundColor3=T.Stroke,BorderSizePixel=0,Position=UDim2.new(0,4,1,-1),Size=UDim2.new(1,-8,0,1),BackgroundTransparency=0.5,Parent=f})
                return f
            end
            ...
            function Tab:AddButton(o)
                o = o or {}
                local f = baseCard(34)
                local b = new("TextButton",{BackgroundTransparency=1,Size=UDim2.new(1,0,1,0),Font=Enum.Font.GothamSemibold,TextSize=13,TextColor3=T.Text,Text=tostring(o.Name or "Button"),AutoButtonColor=false,Parent=f,BorderSizePixel=0})
                b.MouseEnter:Connect(function() tween(f,0.15,{BackgroundColor3=T.Panel2}):Play() end)
                b.MouseLeave:Connect(function() tween(f,0.15,{BackgroundColor3=T.Panel}):Play() end)
                b.MouseButton1Click:Connect(function()
                    tween(f,0.08,{BackgroundColor3=T.Accent2}):Play()
                    task.delay(0.12,function() tween(f,0.15,{BackgroundColor3=T.Panel}):Play() end)
            ...
            function Tab:AddToggle(o)
                ...
                local f = baseCard(o.Description and 46 or 34)
                new("TextLabel",{BackgroundTransparency=1,Position=UDim2.new(0,10,0,0),Size=UDim2.new(1,-70,0,o.Description and 22 or 34),Font=Enum.Font.GothamSemibold,TextSize=13,TextColor3=T.Text,TextXAlignment=Enum.TextXAlignment.Left,TextYAlignment=Enum.TextYAlignment.Center,Text=tostring(o.Name or "Toggle"),Parent=f})
                if o.Description and o.Description~="" then
                    new("TextLabel",{BackgroundTransparency=1,Position=UDim2.new(0,10,0,22),Size=UDim2.new(1,-70,0,18),Font=Enum.Font.Gotham,TextSize=11,TextColor3=T.SubText,TextXAlignment=Enum.TextXAlignment.Left,Text=tostring(o.Description),Parent=f})
                end
                local sw = new("TextButton",{BackgroundColor3=T.Panel2,AnchorPoint=Vector2.new(1,0.5),Position=UDim2.new(1,-10,0.5,0),Size=UDim2.new(0,44,0,20),AutoButtonColor=false,Text="",Parent=f,BorderSizePixel=0})
                corner(10).Parent = sw
                stroke(T.StrokeSoft,1).Parent = sw
                local dot = new("Frame",{BackgroundColor3=T.Muted,Position=UDim2.new(0,2,0.5,-8),Size=UDim2.new(0,16,0,16),Parent=sw,BorderSizePixel=0})
                corner(8).Parent = dot
                local function apply()
                    if state then
                        tween(sw,0.15,{BackgroundColor3=T.Accent2}):Play()
                        tween(dot,0.15,{BackgroundColor3=Color3.new(1,1,1),Position=UDim2.new(1,-18,0.5,-8)}):Play()
                    else
                        tween(sw,0.15,{BackgroundColor3=T.Panel2}):Play()
                        tween(dot,0.15,{BackgroundColor3=T.Muted,Position=UDim2.new(0,2,0.5,-8)}):Play()
                    end
                end
                apply()
                ...
                local api = {}
                function api:Set(v)
                    state = v and true or false; apply()
                    if o.Callback then task.spawn(o.Callback, state) end
                end
                -- Deferred initial callback so scripts (Fast Attack, etc.) fire after full UI + character ready
                if state and o.Callback then
                    task.defer(function() pcall(o.Callback, state) end)
                end
                return api
            end
            ...
            function Tab:AddSlider(o)
                ...
                local f = baseCard(50)
                new("TextLabel",{BackgroundTransparency=1,Position=UDim2.new(0,10,0,4),Size=UDim2.new(1,-70,0,18),Font=Enum.Font.GothamSemibold,TextSize=13,TextColor3=T.Text,TextXAlignment=Enum.TextXAlignment.Left,Text=tostring(o.Name or "Slider"),Parent=f})
                local vlbl = new("TextLabel",{BackgroundTransparency=1,Position=UDim2.new(1,-60,0,4),Size=UDim2.new(0,50,0,18),Font=Enum.Font.GothamBold,TextSize=12,TextColor3=T.Accent,TextXAlignment=Enum.TextXAlignment.Right,Text=tostring(val),Parent=f})
                local bar = new("Frame",{BackgroundColor3=T.Panel2,Position=UDim2.new(0,10,1,-18),Size=UDim2.new(1,-20,0,10),Parent=f,BorderSizePixel=0})
                corner(5).Parent = bar
                stroke(T.StrokeSoft,1).Parent = bar
                local fill = new("Frame",{BackgroundColor3=T.Accent,Size=UDim2.new((val-min)/(max-min),0,1,0),Parent=bar,BorderSizePixel=0})
                corner(5).Parent = fill
                ...
                if o.Default ~= nil and o.Callback then task.defer(function() pcall(o.Callback, val) end) end
                local api={}
                function api:Set(v)
                    val = math.clamp(v, min, max)
                    fill.Size = UDim2.new((val-min)/(max-min),0,1,0); vlbl.Text=tostring(val)
                    if o.Callback then task.spawn(o.Callback, val) end
                end
                return api
            end
            ...
            function Tab:AddDropdown(o)
                ...
                local head = new("Frame",{BackgroundTransparency=1,Size=UDim2.new(1,0,0,34),Parent=f})
                new("TextLabel",{BackgroundTransparency=1,Position=UDim2.new(0,10,0,0),Size=UDim2.new(0.5,-10,1,0),Font=Enum.Font.GothamSemibold,TextSize=13,TextColor3=T.Text,TextXAlignment=Enum.TextXAlignment.Left,Text=tostring(o.Name or "Dropdown"),Parent=head})
                local valTxt = new("TextLabel",{BackgroundTransparency=1,Position=UDim2.new(0.5,0,0,0),Size=UDim2.new(0.5,-30,1,0),Font=Enum.Font.Gotham,TextSize=12,TextColor3=T.Accent,TextXAlignment=Enum.TextXAlignment.Right,Text=tostring(current or "-"),Parent=head})
                local arr = new("TextLabel",{BackgroundTransparency=1,Position=UDim2.new(1,-20,0,0),Size=UDim2.new(0,16,1,0),Font=Enum.Font.GothamBold,TextSize=14,TextColor3=T.Accent,Text="v",Parent=head})
                local btn = new("TextButton",{BackgroundTransparency=1,Size=UDim2.new(1,0,1,0),Text="",Parent=head})
                local list = new("Frame",{BackgroundTransparency=1,Position=UDim2.new(0,8,0,36),Size=UDim2.new(1,-16,0,0),Parent=f})
                new("UIListLayout",{Padding=UDim.new(0,3),Parent=list})

                local function rebuild()
                    for _,c in ipairs(list:GetChildren()) do if c:IsA("TextButton") then c:Destroy() end end
                    for _,opt in ipairs(opts) do
                        local ob = new("TextButton",{BackgroundColor3=T.Panel2,Size=UDim2.new(1,0,0,26),Font=Enum.Font.Gotham,TextSize=12,TextColor3=T.Text,Text=tostring(opt),AutoButtonColor=false,Parent=list,BorderSizePixel=0})
                        corner(6).Parent = ob
                        stroke(T.StrokeSoft,1).Parent = ob
                        ob.MouseEnter:Connect(function() tween(ob,0.12,{BackgroundColor3=T.Accent2}):Play() end)
                        ob.MouseLeave:Connect(function() tween(ob,0.12,{BackgroundColor3=T.Panel2}):Play() end)
                        ob.MouseButton1Click:Connect(function()
                            current = opt; valTxt.Text = tostring(opt)
                            open=false; f.Size = UDim2.new(1,-4,0,34); arr.Text="v"
                ...
                btn.MouseButton1Click:Connect(function()
                    open = not open
                    if open then
                        local h = 34 + 6 + (#opts * 29)
                        f.Size = UDim2.new(1,-4,0,h); arr.Text="^"
                    else
                        f.Size = UDim2.new(1,-4,0,34); arr.Text="v"
                    end
                end)
                if current and o.Callback then task.defer(function() pcall(o.Callback, current) end) end
                local api={}
                function api:Refresh(newOpts, keep)
                    opts = newOpts or {}
                    if not keep then current=nil; valTxt.Text="-" end
                    rebuild()
                    if open then f.Size = UDim2.new(1,-4,0,34+6+(#opts*29)) end
                end
                function api:Set(v) current=v; valTxt.Text=tostring(v); if o.Callback then task.spawn(o.Callback,v) end end
                return api
            end

            function Tab:AddParagraph(title, desc)
                local f = new("Frame",{BackgroundColor3=T.Panel,Size=UDim2.new(1,-4,0,0),AutomaticSize=Enum.AutomaticSize.Y,Parent=page,BorderSizePixel=0})
                corner(8).Parent = f
                stroke(T.StrokeSoft,1).Parent = f
                pad(8).Parent = f
                new("UIListLayout",{Padding=UDim.new(0,2),Parent=f})
                local t = new("TextLabel",{BackgroundTransparency=1,Size=UDim2.new(1,0,0,18),Font=Enum.Font.GothamBold,TextSize=13,TextColor3=T.Accent,TextXAlignment=Enum.TextXAlignment.Left,Text=tostring(title or ""),Parent=f})
                local d = new("TextLabel",{BackgroundTransparency=1,Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,Font=Enum.Font.Gotham,TextSize=12,TextColor3=T.Text,TextXAlignment=Enum.TextXAlignment.Left,TextWrapped=true,Text=tostring(desc or ""),Parent=f})
                local api={}
                function api:SetDesc(txt) d.Text = tostring(txt or "") end
                function api:SetTitle(txt) t.Text = tostring(txt or "") end
                function api:Set(txt) d.Text = tostring(txt or "") end
                return api
            end

            table.insert(Window._tabs, Tab)
            if #Window._tabs == 1 then selectTab(Tab) end
            return Tab
        end
        ...
        local function setHidden(v)
            ...
            if mobileBtn then mobileBtn.Visible = hidden end
            if mobileGui then mobileGui.Enabled = hidden end
        end
        miniBtn.MouseButton1Click:Connect(function() setHidden(true) end)
        closeBtn.MouseButton1Click:Connect(function()
            pcall(function() gui:Destroy() end)
            if mobileGui then pcall(function() mobileGui:Destroy() end) end
        end)

        function Window:NewMinimizer(mcfg)
            mcfg = mcfg or {}
            local Mini = {}
            if mcfg.KeyCode then
                UserInputService.InputBegan:Connect(function(input, gp)
                    if gp then return end
                    if input.KeyCode == mcfg.KeyCode then setHidden(not hidden) end
                end)
            end
            function Mini:CreateMobileMinimizer(opts)
                opts = opts or {}
                -- own ScreenGui, lower DisplayOrder, only enabled when hidden
                mobileGui = new("ScreenGui",{Name="AuroraXMini",ResetOnSpawn=false,ZIndexBehavior=Enum.ZIndexBehavior.Sibling,IgnoreGuiInset=true,Parent=parent,Enabled=false})
                pcall(function() mobileGui.DisplayOrder = 400 end)
                mobileBtn = new("ImageButton",{
                    Name="AuroraMobileBtn",
                    Image = opts.Image or "",
                    BackgroundColor3 = opts.BackgroundColor3 or Color3.fromRGB(255,255,255),
                    BackgroundTransparency = opts.BackgroundTransparency or 0.15,
                ...
                corner(22).Parent = mobileBtn
                stroke(T.Stroke,1.5).Parent = mobileBtn
                drag(mobileBtn, mobileBtn)
                local pressed=false
                mobileBtn.MouseButton1Click:Connect(function()
                    if pressed then return end
                    pressed=true; setHidden(false); task.wait(0.15); pressed=false
                end)
                return mobileBtn
            end
            return Mini
...
local MobileButtonIcon = "rbxassetid://95722261449054"
local MobileButton = Minimizer:CreateMobileMinimizer({
  Image = MobileButtonIcon,
...
local Tabs = {
    ...
    SeaEvent = Window:MakeTab({ Title = "Sea Event", Icon = "rbxassetid://100417820243649" }),
    Race = Window:MakeTab({ Title = "Mirage And Race", Icon = "rbxassetid://11162889532" }),
    Prehistoric = Window:MakeTab({ Title = "Volcano Event", Icon = "rbxassetid://77424026081298" }),
...
local DiscordLink = "https://discord.gg/9KhYDxXMQP"
...
Tabs.Info:AddButton({
    ...
    Callback = function()
        ...
        if OrionLib and OrionLib.MakeNotification then
            OrionLib:MakeNotification({ Name = "Discord", Content = "Link copied!", Time = 3 })
        end
    end
})
...
Tabs.Info:AddButton({
    ...
    Callback = function()
        if setclipboard then setclipboard(WhatsAppLink) end
        if OrionLib and OrionLib.MakeNotification then
            OrionLib:MakeNotification({ Name = "WhatsApp", Content = "Link copied!", Time = 3 })
        end
    end
})
